...
class Thread_Base
{
 ...
 public:
    //@brief 获取线程名
    inline const char * get_thread_name() { return m_thread_name; }
    //@brief 设置线程名
 ...
 private:
    //@brief 设置线程状态; 参数见宏定义THREAD_SS_XXX
    inline void set_thread_status(int thread_status) { m_thread_status=thread_status; }
 private:
