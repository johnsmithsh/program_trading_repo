...
TEST(RttiFieldDescriptor, RttiFieldDescriptor)
{
   ...
   ST_FIELD_DESC field_vec[]={
     {0, RDT_CHAR,  "bs_flag",     "买卖方向",    1, 0,  0, 1},
     {1, RDT_CHAR,  "sh_flag",    "投保标记",     1, 1,  1, 1},
     {2, RDT_CHAR,  "eo_flag",    "开平标记",     1, 2,  2, 1},
     {3, RDT_UCHAR, "if_success", "成功标记",     1, 3,  3, 1},
     {4, RDT_INT2,  "core_no",    "交易核心编码", 1, 4,  4, 2},
     {5, RDT_UINT2, "umask",      "委托掩码",     1, 6,  6, 2},
     {6, RDT_INT4,  "entrust_qty", "委托手数",    1, 8,  8, 4},
     {7, RDT_UINT4, "done_qty",   "成交手数",     1, 12, 12, 4},
     {8, RDT_LONG,  "serial_no",  "委托流水号",   1, 16, 16, 8},
     {9, RDT_STR,   "cust_no",    "客户号",       1, 24, 24, 16}
   };
   int rc;
   ...
   EXPECT_EQ(serial_size, field_vec[9].offset   + field_vec[9].size);
   EXPECT_EQ(st_size,     field_vec[9].st_offset + field_vec[9].size);
   ...
   index=0;
   strcpy(name, "bs_flag");
   ptr_byindex = field_demo.get_field_ptr(index);
   ptr_byname = field_demo.get_field_ptr(name);
   ...
   EXPECT_LT(rc,0);
   ptr=field_demo.get_field_ptr("bs_flag");
   ...
   EXPECT_LT(rc,0);
   ptr=field_demo.get_field_ptr("entrust_qty");
   ...
   EXPECT_LT(rc,0);
   ptr=field_demo.get_field_ptr("cust_no");
   ...
   printf("\n\n测试案例: 引用修改...\n");
   index=3;
   ST_FIELD_DESC &field_ref = field_demo.get_field(index);
   char field_comment[64]="成功标记(mod by ref)";
   strcpy(field_ref.field_comment,field_comment);
   ptr=field_demo.get_field_ptr(index);
   ...
   printf("\n\n测试案例: 指针修改...\n");
   index=4;
   ST_FIELD_DESC *mod_ptr = field_demo.get_field_ptr(index);
