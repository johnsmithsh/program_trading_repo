...
//         size不足,则返回字符串被截断
//返回值: 0-成功; <0失败
int os_path_basename(char *path, char *basename, int size)
{
   *basename='\0';
   if( (NULL==path) || ('\0'==*path) ) return -1;
   char *ptr=NULL;
...
//功能: 判断path是文件
inline int os_path_isfile(char *path)
{
   struct stat info;
   int rc;
...
//功能: 判断path是目录
inline int os_path_isdir(char *path)
{
   struct stat info;
   int rc;
   memset(&info, 0, sizeof(info));
...
inline int os_path_isabs(char *path)
{
...
inline int os_path_file_getsize(char *path)
{
   int rc=0;
...
//返回值:
//    0-成功; <0-失败;
inline int os_path_mkdir(char *path)
{
...
inline int os_path_force_removedir(char *path);
